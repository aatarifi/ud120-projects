...
if len(cleaned_data) > 0:
    ages_train_cleaned, net_worths_train_cleaned, errors = zip(*cleaned_data)   
    ages_train_cleaned       = numpy.reshape( numpy.array(ages_train_cleaned), (len(ages_train_cleaned), 1))
    net_worths_train_cleaned = numpy.reshape( numpy.array(net_worths_train_cleaned), (len(net_worths_train_cleaned), 1))
    ...
    try:
        ...
        plt.plot(ages_train_cleaned, reg.predict(ages_train_cleaned), color="blue")
    except NameError:
        print "you don't seem to have regression imported/created,"
        print "   or else your regression object isn't named reg"
        print "   either way, only draw the scatter plot of the cleaned data"
    plt.scatter(ages_train_cleaned, net_worths_train_cleaned)
    plt.xlabel("ages")
    plt.ylabel("net worths")
    plt.show()
    print "333333333: ",len(cleaned_data)
    print "444444444: ",len(ages_train_cleaned) + len(net_worths_train_cleaned)
    print "555555555: ",len(net_worths_train_cleaned)
    print "666666666: ",max(errors)
    print "errors type: : ",type(errors)
    #print cleaned_data[89][1]
    
    # Lesson 8_12 Slope after cleaning :  What is the new slope? 
    print "New slope after cleaning: ",reg.coef_
    
    # Lesson 8_13 What’s the new score when you use the regression to make predictions on the test set?
    # After the outilers have been cleaned away
    reg.fit(ages_train_cleaned, net_worths_train_cleaned)
    plt.plot(ages_test, reg.predict(ages_test), color="red")
    plt.scatter(ages_train_cleaned, net_worths_train_cleaned)
